...
function SidebarClass({ data, match, defaultUri }) {
  ...
  const list = [];
  data.chapter.forEach((chptr, index) => {
    list.push(
      <li key={`${chptr.course_id}-${index}`}>
        <span className="nav-header relative block py-3 px-5 bg-indigo-800 text-white text-left">
          {chptr?.name ?? "Chapter name"}
        </span>
      </li>
    );
    if (chptr?.lessons?.length > 0)
      chptr.lessons.forEach((lesson, index2) => {
        list.push(
          <li key={`${chptr.course_id}-${lesson.id}-${index2}`}>
            <Link
              className={[
                "relative flex items-center py-3 px-5 transition-all duration-200 w-full text-left truncate ...",
                getNavLinkClass(
                  `/courses/${data.id}/${chptr.id}/${lesson.video}`
                ),
              ].join(" ")}
              to={`/courses/${data.id}/${chptr.id}/${lesson.video}`}
            >
